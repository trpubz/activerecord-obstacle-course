...
# ----------------------
  ...
  it '22. calculates the total sales for all but one user' do
    ...
    # ------------------ Using ActiveRecord ---------------------
    total_sales = User.where.not(id: @user_2.id).joins(:orders).sum(:amount)
    # -----------------------------------------------------------
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.joins(:items).where(user_id: @user_2.id, items: @item_4.id)
    # -----------------------------------------------------------
  ...
  it '25. returns items that are associated with one or more orders' do
    ...
    Order.joins(:items).distinct.pluck("items.name")
    # ---------------------------------------------------------------
