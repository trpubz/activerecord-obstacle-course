...
# ----------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    users = User.joins(:items).where("items.id = #{@item_8.id}").distinct(:name).order(:name).pluck(:name)
    # ------------------------------------------------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    items_for_user_3_third_order = User.find(@user_3.id).orders[2].items.pluck("items.name")
    # ------------------------------------------------------------
